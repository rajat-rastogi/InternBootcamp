...
AllJoynCarWatcher::AllJoynCarWatcher(AllJoynBusAttachment^ busAttachment) :
...
AllJoynCarWatcher::~AllJoynCarWatcher()
{
    UnregisterFromBus();
}

void AllJoynCarWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.Sample.AllJoynCar" };
...
void AllJoynCarWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.microsoft.Sample.AllJoynCar"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.microsoft.Sample.AllJoynCar"),
            port);
        Added(this, args);
    }
...
void AllJoynCarWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<AllJoynCarWatcher>
        };
...
void AllJoynCarWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_AllJoynCarIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &AllJoynCarWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void AllJoynCarWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void AllJoynCarWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
